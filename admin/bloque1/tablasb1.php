...
if ($last_id !== null) {
    $querybloque1 = "
        ...
        WHERE dp.idpedimento = $last_id
    ";

    $resultbloque1 = $conexion->query($querybloque1);

    if ($resultbloque1 && $resultbloque1->num_rows > 0) {
        $datosb1 = $resultbloque1->fetch_assoc();
        ?>
        <table class="table table-bordered table-hover">
            <tr>
                ...
                <th>CVE PEDIMENTO</th>
                <td><?php echo htmlspecialchars($datosb1['clave_apendice2']); ?></td>
                <th>REGIMEN</th>
                <td><?php echo htmlspecialchars($datosb1['clave_apendice16']); ?></td>
            </tr>
        </table>

        <?php
    } else {
        // Si no se encontraron registros o hubo un error en la consulta, la tabla aparecerá vacía 
        ?>
        <table class="table table-bordered table-hover">
            <tr>
                <th>NUM.PEDIMENTO</th>
    ...
    // El parámetro 'id' no está definido en la URL, la tabla aparecerá vacía ya que no tiene ningún id la URL 
    ?>
    <table class="table table-bordered table-hover">
        <tr>
            <th>NUM.PEDIMENTO</th>
