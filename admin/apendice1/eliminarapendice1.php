...
if (isset($_GET['idapendice1'])) {
    // Obtener el ID del registro a eliminar
    $idapendice1 = mysqli_real_escape_string($conexion, $_GET['idapendice1']);

    // Validar que el ID no esté vacío
    if (!empty($idapendice1)) {
        // Eliminar el registro de la base de datos
        $query = "DELETE FROM apendice1 WHERE idapendice1='$idapendice1'";

        if (mysqli_query($conexion, $query)) {
            // Redirigir a una página de éxito o mostrar un mensaje de éxito
            header("Location: ../apendice1.php?success=Complemento del apéndice 1 eliminado exitosamente.");
            exit();
        } else {
        ...
        header("Location: ../apendice1.php?error=ID del complemento es obligatorio.");
        exit();
    }
} else {
